...
class DB:
    ...
    def delete(self, query=None, table=None, attributes=None, values=None):
        if not query:
            wildcards = self.wildcars(len(values))
            attributes_to_str = ", ".join(attributes)
            query = """DELETE FROM {} WHERE  {} = ( {} ) """.format(table, attributes_to_str, wildcards)
            values = [x.strip(' ') for x in values]
            values = tuple(values)
    ...
    def select_from_table(self):
        try:
            db = pymysql.connect(host=self._host, user=self._username, password=self._password, database=self._database)
            cursor = db.cursor()
